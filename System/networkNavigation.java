...
public class networkNavigation {
    /*
     * 
     * This is a direct implementation of BFS, just takes a depth level and a user
     * id. and returns a tuple
     * datastructure that is sorted according to the people the user isn't friends
     * with but have either friends
     * or friends of friends in common (depending on the depth level taken as a
     * parameter).
     * -----------------------------------------------------------------------------
     * -------------------
     * Will also handle post propagation using the same method, taking the depth
     * level and a post object
     * and posting it other users fields that are applicable to the constraints and
     * also not restricted users.
     * -----------------------------------------------------------------------------
     * -------------------------
     * Also the current intuition to handle restricted users is add a bitset to each
     * user and if a user is
     * restricted we'll just have his the indx corresponding to his id toggled on
     * and just check while doing
    ...
    TreeSet<Tuple> suggestFriends(int depth) {
